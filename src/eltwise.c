    ...
    for (int ih = 0; ih < h; ih++) {
        for (int iw = 0; iw < w; iw++) {
            for (int ic = 0; ic < c; ic++) {
                int64_t a, b;
                if (is_int16) {
                    a = (int64_t)tensor_get_i16(input_a, ih, iw, ic);
                    b = (int64_t)tensor_get_i16(input_b, ih, iw, ic);
                } else {
                    a = (int64_t)tensor_get_i8(input_a, ih, iw, ic);
                    b = (int64_t)tensor_get_i8(input_b, ih, iw, ic);
                }
